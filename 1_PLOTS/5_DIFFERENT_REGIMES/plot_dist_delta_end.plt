...
fit f(x) 'distribution_DELTA4_KAPPA0P1_end' using (log($1)):(log($2)) via a, c 
fit g(x) 'distribution_DELTA4_KAPPA0P5_end' using (log($1)):(log($2)) via b, d
fit h(x) 'distribution_DELTA4_KAPPA1_end' using (log($1)):(log($2)) via k, m
